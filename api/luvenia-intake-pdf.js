// GET /api/luvenia-intake-pdf?id=<intakeId>
//
// Streams the branded PDF of Luvenia's submitted intake as
...
export default async function handler(req, res) {
  ...
  // (intake URL is texted privately to Luvenia). When env var is set, the
  // token check is enforced as designed.
  //
  // 2026-05-17 fix: matches the softening in /api/luvenia-intake.js — both
  ...
  if (expectedToken) {
    ...
    console.warn('luvenia-intake-pdf: LUVENIA_INTAKE_SECRET not set — running ungated.');
  }
  ...
  try {
    ...
    const pdfBuffer = await generateLuveniaPDF({
      answers: record.answers,
      submittedAt: record.submittedAt || new Date().toISOString(),
    });

    res.setHeader('Content-Type', 'application/pdf');
    res.setHeader('Content-Disposition', 'inline; filename="luvenia-intake.pdf"');
    res.setHeader('Cache-Control', 'private, no-store');
    return res.status(200).send(pdfBuffer);
  } catch (err) {
    console.error('luvenia-intake-pdf: error', err);
