// POST /api/luvenia-intake
//
// Receives Luvenia N Truss's pre-call intake answers, stores them in
...
// at /api/luvenia-intake-pdf?id=<intakeId>.

import { Resend } from 'resend';
import { kv } from '@vercel/kv';
import { SECTION_MAP, formatAnswer } from './_luvenia-schema.js';
import { generateLuveniaPDF } from './_luvenia-pdf.js';
    ...
    <tr><td align="center" style="padding:24px 16px;">
      <table role="presentation" width="100%" cellpadding="0" cellspacing="0" style="max-width:720px;background:#FFFDF7;border-radius:14px;border:1px solid #E6DECE;">
        <tr><td style="padding:24px 24px 8px;">
          ...
          <h1 style="font-family:Georgia,serif;font-size:22px;margin:6px 0 4px;color:#2C2A26;">Luvenia N Truss</h1>
...
export default async function handler(req, res) {
  ...
  // 2026-05-14 hardening (audit P0-1): require LUVENIA_INTAKE_SECRET token
  // to gate this endpoint when the secret is configured. Without a secret
  // configured this is a single-named-recipient form (Luvenia) reachable
  // only via the private `/luvenia` URL Joel texts her — URL-obscurity is
  ...
  // warn-and-allow. The hard-500 cost Luvenia her Sunday-night submission
  ...
  const expectedToken = process.env.LUVENIA_INTAKE_SECRET || '';
  if (expectedToken) {
    const supplied =
    ...
    console.warn('luvenia-intake: LUVENIA_INTAKE_SECRET not set — running ungated. Wire the env var in Vercel to re-enable the token check.');
  }
  ...
  if (process.env.KV_REST_API_URL) {
    try {
      await kv.set(`luvenia-intake:${intakeId}`, { intakeId, answers, submittedAt }, { ex: 90 * 86400 });
      // Also store as the "latest" pointer for easy lookup
      await kv.set('luvenia-intake:latest', { intakeId, submittedAt }, { ex: 90 * 86400 });
    } catch (err) {
      console.error('luvenia-intake: KV store failed', err.message);
    }
  }
  ...
  let pdfBuffer = null;
  try {
    pdfBuffer = await generateLuveniaPDF({ answers, submittedAt });
  } catch (err) {
    console.error('luvenia-intake: PDF generation failed', err.message);
  ...
  try {
    await getResend().emails.send({
      ...
      subject: `[Luvenia BP Triangle Intake] Submitted — for the Jun 9 call`,
      html: renderEmailHtml(answers, submittedAt),
      attachments: pdfBuffer ? [{
        filename: 'luvenia-intake.pdf',
        content: pdfBuffer.toString('base64'),
      }] : undefined,
    });
  } catch (err) {
    console.error('luvenia-intake: email send failed', err.message);
