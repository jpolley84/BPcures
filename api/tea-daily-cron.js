...
function renderTable(rowsHtml) {
  return `
    <table style="border-collapse:collapse;width:100%;margin-bottom:8px;">
      ...
      ${rowsHtml || `<tr><td colspan="6" style="padding:10px;color:#9A9186;font-size:13px;font-style:italic;">None right now.</td></tr>`}
    </table>`;
}
...
export default async function handler(req, res) {
  ...
  <div style="font-family:-apple-system,Segoe UI,sans-serif;max-width:820px;margin:0 auto;">
    <h2 style="color:#121110;margin-bottom:4px;">Steady tea — fulfillment digest (${day})</h2>
    <p style="color:#7A7061;font-size:12.5px;margin-top:0;">One email, three lists. Mark an order fulfilled by telling Joel's assistant the Order # or buyer name/email — it moves to the Fulfilled list starting with tomorrow's digest.</p>

    <h3 style="color:#B85A36;margin:22px 0 4px;">1. New — last 24 hours (${newOrders.length})</h3>
    <p style="color:#2B2824;font-size:13px;margin:0 0 8px;">Total: <strong>$${(newTotalCents / 100).toFixed(2)}</strong></p>
    ...
    <p style="color:#2B2824;font-size:13px;margin:0 0 8px;">Everything not yet marked fulfilled. Total: <strong>$${(openTotalCents / 100).toFixed(2)}</strong></p>
    ...
    <p style="color:#7A7061;font-size:12px;margin-top:16px;">Automated digest from bpquiz.com/tea. Addresses are exactly as the buyer entered them at checkout. Subscription renewals do not appear here; ship those from the Stripe subscriptions dashboard.</p>
  </div>`;

  const text =
    `Steady tea — fulfillment digest (${day})\n` +
    `One email, three lists. Tell Joel's assistant an Order # or buyer name/email to mark it fulfilled.\n\n` +
    renderTextSection(`1. NEW — last 24 hours (${newOrders.length}), total $${(newTotalCents / 100).toFixed(2)}`, newOrders) + '\n' +
